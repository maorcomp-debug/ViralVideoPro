...
export async function deactivateShareLink(token: string): Promise<void> {
  ...
  const res = await fetch(`/api/share?action=deactivate&token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${session.access_token}`,
    },
...
export async function fetchPublicShare(token: string) {
  const s = getShareStrings();
  const res = await fetch(`/api/share?action=public&token=${encodeURIComponent(token)}`);
  const json = await res.json().catch(() => ({}));
  if (!res.ok) {
