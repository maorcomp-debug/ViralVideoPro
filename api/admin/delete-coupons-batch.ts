...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!Array.isArray(couponIds) || couponIds.length === 0) {
      return res.status(400).json({ ok: false, error: 'Missing or empty couponIds array' });
    }

    // Delete redemptions first for all coupons
    ...
    console.error('delete-coupons-batch error:', e);
