...
- **כדי שלא יגיעו מיילים מ-Supabase:** ב-Supabase יש להפסיק את שליחת מייל האימות המובנית דרך **Auth Hook** (Customize Auth / Auth Hooks) שמבטל את השליחה, **בלי** לכבות "Confirm email". אז יגיע רק המייל מהאפליקציה (עיצוב ממורכז).
