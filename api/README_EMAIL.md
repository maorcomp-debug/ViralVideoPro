...
## מייל אימות בהרשמה – מקור יחיד (הגדרה מחייבת)

**מייל אימות החשבון נשלח רק מהאפליקציה** דרך `api/send-confirmation-email` (Resend), בעיצוב Viraly: רקע כהה, כפתור צהוב "כניסה לחשבון", "פתח קישור אימות". זה המייל היחיד שצריך להתקבל באימות.

### למניעת שני מיילים (אימות כפול)

אם מתקבלים **שני** מיילי אימות – הנכון הוא זה מעיצוב Viraly (כפתור צהוב, רקע כהה). המייל השני הוא המייל המובנית של Supabase.

כדי לקבל **רק** את המייל הנכון: ב-**Supabase** יש להפסיק את שליחת מייל האימות המובנית, אך **בלי** לכבות "Confirm email" (כי אז משתמשים יוכלו להיכנס בלי אימות). הדרך: להשתמש ב-**Auth Hook** ב-Supabase שמבטל את שליחת המייל המובנית (Customize Auth / Auth Hooks בתיעוד Supabase). אחרי ההגדרה, רק המייל מ־`send-confirmation-email` (Resend) יישלח, ולחיצה על הקישור תאמת ותכניס לחשבון.
