...
function subscriptionEscapeHtml(text: string): string {
  const map: Record<string, string> = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' };
  return text.replace(/[&<>"']/g, (c) => map[c] || c);
}

const SUBSCRIPTION_EMAIL_STRINGS: Record<EmailLang, {
  welcomeLine: string;
  confirmPause: string;
...
function getSubscriptionSubject(action: 'pause' | 'cancel' | 'resume', lang: EmailLang): string {
  const s = SUBSCRIPTION_EMAIL_STRINGS[lang];
  if (action === 'pause') return s.subjectPause;
  if (action === 'cancel') return s.subjectCancel;
  return s.subjectResume;
}

function getSubscriptionConfirmLine(action: 'pause' | 'cancel' | 'resume', lang: EmailLang): string {
  const s = SUBSCRIPTION_EMAIL_STRINGS[lang];
...
function buildSubscriptionActionEmailHtml(confirmLine: string, lang: EmailLang): string {
  const s = SUBSCRIPTION_EMAIL_STRINGS[lang];
...
<body style="margin:0;padding:0;background:#1a1a1a;font-family:Arial,sans-serif;direction:${dir};text-align:${align};">
  <div style="max-width:600px;margin:0 auto;padding:24px;background:#1a1a1a;color:#fff;">
    <p style="margin:0 0 24px 0;line-height:1.6;color:#fff;">${s.welcomeLine}</p>
    <p style="margin:0 0 32px 0;line-height:1.6;color:#fff;">${subscriptionEscapeHtml(confirmLine)}</p>
    <p style="margin:0;font-size:0.85rem;color:#888;text-align:${align};">Viraly – Video Director Pro</p>
  </div>
</body>
</html>`;
}
...
async function handleShareApi(req: VercelRequest, res: VercelResponse) {
